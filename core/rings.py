...
- plot_ring_fcurves_grouped_stable :
    Generates Figure 3 - all distinct f-curves grouped by symmetry and annotated
    with stability markers determined from the Jacobian spectrum.
- plot_all_phase_locked_solutions_for_varying_w0_both :
    Produces Figure 4a - shows the evolution of f-curves for different omega_0 values,
    with stable and unstable intersections marked.
- plot_bifurcation_ring_branches :
    Produces Figure 4b - bifurcation diagram tracking stable branches of zeta as omega_0 varies.
...
def find_what_works_ring(S_plus, S_minus, B1, K1, C1, tol=1e-7, omega=None, z_range=None, verbose=False):
    """
...
def generate_multiset_label(S_plus, S_minus):
    return rf"$(|S_{{[0]}}^\bullet|, |S_{{[0]}}^\circ|)=({len(S_plus)},{len(S_minus)})$"
...
def plot_ring_fcurves_grouped_stable(B1, K1, C1, all_partitions, omega=None, resolution=600, levels=(-4, 4), save_path="Fig3.pdf"):
    """
    ...
    for S_plus, S_minus in all_partitions:
        ...
        if not found:
            j = len(unique_curves)
            unique_curves.append({'zetas': zetas, 'f': f_vals, 'S_plus': S_plus, 'S_minus': S_minus})
            label_groups[j].append(generate_multiset_label(S_plus, S_minus))
    ...
    for i, cur in enumerate(unique_curves):
        ...
        f_flat = f_vals.ravel()
        label = generate_multiset_label(S_plus, S_minus)
        ...
        for k in range(levels[0], levels[1] + 1):
            ...
            for idx in idxs:
                ...
                if stable:
                    ax.plot(z_star, k, "o", color=colors[i], markersize=11, markeredgewidth=1.2, markeredgecolor=colors[i], markerfacecolor=colors[i], zorder=3)
                else:
                    ax.plot(z_star, k, "o", color=colors[i], markersize=11, markeredgewidth=1.2, markeredgecolor=colors[i], markerfacecolor="white", 
                            alpha=0.95, zorder=3)

    # Cosmetics
    ax.set_xlabel(r"$\zeta^{[-]}$", fontsize=16)
    ax.set_ylabel(r"$1^\top f_{0}(\psi^{[-]})/(2\pi)$", fontsize=16)
    # Legend
    ...
    for h, l in zip(handles, labels):
        if l not in seen:
            ...
            seen.add(l)
    ax.legend(uniq_handles, uniq_labels, bbox_to_anchor=(0.5, -0.12), loc="upper center", ncol=2, frameon=False, fontsize=14)
    plt.xticks(fontsize=14)
    plt.yticks(fontsize=14)
    
    ax.text(-0.08, 0.97, "a", transform=ax.transAxes, ha="left", va="top", fontsize=16, weight="bold", clip_on=False)
...
def plot_all_phase_locked_solutions_for_varying_w0_both(Sp0, Sm0, Sp1, Sm1, B1, K1, C1, w0_values_main, w0_values_inset,
                                                        resolution_main=1500, resolution_inset=3500, save_path="Fig4a.pdf"):
    """
    Plot main and inset figures for varying omega_0 values (Fig. 4a).
    ...
    for w0 in w0_values_main:
        ...
        for idx, (z, zeta) in enumerate(roots_0):
            ax.plot(zeta, z, 'o', markerfacecolor=color if idx in stable_0 else 'white', markeredgecolor=color, markersize=11,
                    markeredgewidth=1.2, zorder=3)
        for idx, (z, zeta) in enumerate(roots_1):
            ax.plot(zeta, z, 'o', markerfacecolor=color if idx in stable_1 else 'white', markeredgecolor=color, markersize=11,
                    markeredgewidth=1.2, zorder=3)
    ...
    ax.set_xlabel(r"$\zeta^{[-]}$", fontsize=16)
    ax.set_ylabel(r"$1^\top f_0(\psi^{[-]})/(2\pi)$", fontsize=16)
    ...
    for w0 in w0_values_inset:
        ...
        for idx, (z, zeta) in enumerate(roots_0):
            axins.plot(zeta, z, 'o', markerfacecolor=color if idx in stable_0 else 'white', markeredgecolor=color, markersize=11, zorder=3)
        for idx, (z, zeta) in enumerate(roots_1):
            axins.plot(zeta, z, 'o', markerfacecolor=color if idx in stable_1 else 'white', markeredgecolor=color, markersize=11, zorder=3)
    ...
    # Final legend
    ax.legend(lines, labels, loc='upper left', fontsize=13, ncol=2)

    ax.text(-0.13, 0.97, "a", transform=ax.transAxes, ha="left", va="top", fontsize=16, weight="bold", clip_on=False)
    plt.tight_layout()
    plt.savefig(save_path, bbox_inches='tight')
    plt.show()


# === Fig 4b related functions ===
def plot_bifurcation_ring_branches(Sp0, Sm0, Sp1, Sm1, B1, K1, C1, omega0_vals=np.linspace(1.307, 1.3275, 200), target_z=0,
                                   save_path="Fig4b.pdf"):
    """
    Plot the bifurcation diagram (Fig. 4b) showing how the phase-locked roots
    ...
    for i, w0 in enumerate(omega0_vals):
        ...
        results_partitioned.append({"omega0": w0, "roots_5_1": [r[1] for r in roots_5_1], "roots_6_0": [r[1] for r in roots_6_0]})
        print(f"{i}", end=" ")
    ...
    plt.plot(omega_list, root_6_0_a, label=r'$\cos(\theta^{[-]}_{5})>0,\ \mathrm{stable}$', color="#E69F00")

    # Partition S_{[0]}^{\circ} = [5,6]
    plt.plot(omega_list, root_5_1_b, label=r'$\cos(\theta^{[-]}_{5})<0,\ \mathrm{stable}$', color="#003366") #old label=r'$S_{[0]}^{\circ} = [5, 6]$
    plt.plot(omega_list, root_5_1_a, label=r'$\cos(\theta^{[-]}_{5})<0,\ \mathrm{unstable}$', color="#003366", linestyle='--') #old label=r'$S_{[0]}^{\circ} = [5, 6]$'
    ...
    plt.legend(fontsize=13, loc='upper center', bbox_to_anchor=(0.32, 0.73))

    ax = plt.gca()
    # Remove whitespace on the right
    ax.set_xlim(left=omega_list[0], right=omega_list[-1])
    ax.margins(x=0)
    plt.subplots_adjust(bottom=0.25)
    plt.tight_layout()

    # Nice x-ticks
    xticks = np.linspace(omega_list[0], omega_list[-1], 7)
    plt.xticks(xticks)
    ax.xaxis.set_major_formatter(FormatStrFormatter('%.3f'))
    ax.text(-0.13, 0.97, "b", transform=ax.transAxes, ha="left", va="top", fontsize=16, weight="bold", clip_on=False)

    # Saddle-node bifurcation annotation
    ax.annotate("saddle-node\nbifurcation", xy=(omega_list[first_nan_51], root_5_1_b[first_nan_51 - 1]), xycoords="data",
                xytext=(omega_list[first_nan_51] + 0.0018, -0.104), textcoords="data", ha="left", va="center", fontsize=13,
